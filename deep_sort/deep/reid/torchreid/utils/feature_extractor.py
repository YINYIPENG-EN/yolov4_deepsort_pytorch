...
class FeatureExtractor(object):
    ...
    def __init__(
        ...
        if verbose:
            num_params, flops = compute_model_complexity(  # 这个函数主要是计算模型参数
        ...
        if model_path and check_isfile(model_path):
            load_pretrained_weights(model, model_path)  # 加载权重

        # Build transform functions
        transforms = []
        transforms += [T.Resize(image_size)]  # 对图像shape进行resize
        transforms += [T.ToTensor()]
        if pixel_norm:
            transforms += [T.Normalize(mean=pixel_mean, std=pixel_std)] # 求均值和方差
        preprocess = T.Compose(transforms) # 对图像进行预处理
        ...
        # Class attributes
        self.model = model
        self.preprocess = preprocess  # 预处理
        self.to_pil = to_pil
        self.device = device
    ...
    def __call__(self, input):
        ...
        with torch.no_grad():
            features = self.model(images)  # 将图像放入网络得到输出特征
